...
class CogVideoXAttnProcessor2_0:
    ...
    def __call__(
        ...
        query_image = rearrange(query_image, "B N (T S) C -> (B S) N T C", N=attn.heads, T=num_frames, S=spatial_dim,
                                C=head_dim)
        key_image = rearrange(key_image, "B N (T S) C -> (B S) N T C", N=attn.heads, T=num_frames, S=spatial_dim,
                              C=head_dim)
        scale = head_dim ** -0.5
        ...
        batch_size_reshape, num_tokens, num_frames, _ = attn_temp.shape
        ...
        return hidden_states, encoder_hidden_states, mean_scores_mean
